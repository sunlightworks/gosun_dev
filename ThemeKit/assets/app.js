...
$(function() {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if(quantity > 0 && quantity != updated_quantity) {
          ...
          var warning = '<div class="alert alert--error alert--is-visible js-alert col-12 position-relative" role="alert"><div class="flex items-center"><i class="icon--sm material-icons-outlined margin-right-xxs">error_outline</i><span class="text-sm">' + 'Only ' + updated_quantity + ' ' + items_left_text + '</span></div></div>';
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        var line_id = index + 1;
        cart_items_html += '<li class="dr-cart__product cart_item">';
        if (item.image) {
          cart_items_html += '<a class="dr-cart__img" href="' + item.url + '">' +
            '<img src="' + item.image.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '') + '" alt="' + htmlEncode(item.title) + '" />' +
          '</a>';
        }

        cart_items_html += '<section class="flex flex-column">';

        cart_items_html += '<h2 class="text-base"' + 
          '<a class="color-inherit" href="' + item.url +'">' + item.title + '</a>' +
        '</h2>';

        cart_items_html += '<p class="text-sm color-contrast-medium margin-top-xxxs">' + item.product_type + '</p>';

        cart_items_html += '<div class="margin-top-xxxxs left product-quantity-box">' +
            '<span class="ss-icon product-minus js-change-quantity" data-func="minus"><span class="icon-minus"></span></span>' +
            '<input type="number" min="0" class="quantity" name="updates[]" id="updates_' + item.id + '" value="' + item.quantity +'" data-line-id="' + line_id +'" />' +
            '<span class="ss-icon product-plus js-change-quantity" data-func="plus"><span class="icon-plus"></span></span>' +
          '</div>';

        cart_items_html += '</section>';

        cart_items_html += '<section class="flex flex-column align-end justify-between height-100%">' +
          '<div class="reset drawer__close-btn dr-cart__remove-btn">' +
            '<i class="dr-cart__remove-btn icon--sm material-icons-outlined color-contrast-low">delete</i></div>' +
          '<summary class="flex flex-column text-right">';
        
        cart_items_html += '<span class="money color-contrast-higher">' + Shopify.formatMoney(item.price, $('body').data('money-format')) + '</span>'; 
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if(variant[0] && variant[0].compare_at_price > item.price) {
                cart_items_html += ' <span class="money was_price mtn5">' + Shopify.formatMoney(variant[0].compare_at_price, $('body').data('money-format')) + '</span>';
        ...
        cart_items_html += '</summary></section></li>';

      });

      
      let gift_value = 10000;
      let free_shipping = 5000;
      let cart_total = cart.total_price;
      let gift_value_left = gift_value - cart_total;

      if((gift_value_left > 0 ) && (cart.item_count > 0 )) {
        cart_gift_html = "Free 3-ply-cotton-mask on orders $100+ <br><a href='/collections/todays-deals' class='color-success-dark'> Add " + Shopify.formatMoney(gift_value_left, $('body').data('money-format')) + " to claim your free gift ➞</a>";
      } else {
        cart_gift_html = "You've unlocked a free 3-ply-cotton-mask! <br>(Refresh the page if it doesn't appear in your cart)";
      }
      
      cart_action_html += '<span>Subtotal</span><span class="money">' + Shopify.formatMoney(cart.total_price, $('body').data('money-format')) + '</span>';

      if(cart_total >= free_shipping ) {
        cart_shipping_html += '<span>Shipping</span> <span>Free</span>';
      } else {
        cart_shipping_html += '<span>Shipping</span> <span>Calculated at checkout</span>';
      }
      
      if(total_savings > 0 ) {
        cart_savings_html = "You save &nbsp;" + Shopify.formatMoney(total_savings - discounted_price_total, $('body').data('money-format'));
      } else {
        cart_savings_html = "";
      }
    }
    ...
    $('.js-cart_shipping').html(cart_shipping_html);
    $('.js-cart_savings').html(cart_savings_html);
    $('.js-cart_gift').html(cart_gift_html);
  ...
  $(".cart_content .dr-cart__remove-btn").on("click", function() {
    ajaxUpdateCart($(this).data('line-id'), '0', $(this).parents('.cart_content'));
  });
  
  // $(".dr-cart__remove-btn").on("click", function() {
  //   var id = $(this).attr("id");
  ...
  // });
    ...
    $(document).on('click', '.ajax-submit', function(e) {
      ...
      const drawer = document.querySelector('#drawer-cart-id');
      ...
      $.ajax({
        ...
        success: function(itemData) {
          ...
          $addToCartBtn.find('.checkmark').addClass('checkmark-active');
          drawer.classList.add('drawer--is-visible');
